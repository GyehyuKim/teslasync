...
- [`pi/clipserver.py`](pi/clipserver.py) — **Python 표준 라이브러리만** 사용
  ...
  - `GET /healthz` — 배포 후 프로세스/루트 접근 셀프체크
...
- **검증 상태**: `python3 pi/test_clipserver.py` 17개 테스트 통과(healthz, 목록·정렬·메타
  폴백·Range 206/416·경로 탈출 차단). **보드 배포·실차 데이터 검증은 미완**
- 배포물: `pi/clipserver.service` + `pi/install_clipserver.sh`
  (`sudo pi/install_clipserver.sh` → `/mutable` 설치, systemd enable/start, `/healthz` 확인)
