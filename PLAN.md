...
### Phase 1 — 파일 목록/다운로드 API ✅ 구현됨 (2026-07-12, 보드 배포는 아직)
- [`pi/clipserver.py`](pi/clipserver.py) — **Python 표준 라이브러리만** 사용
  (Flask 후보였으나 보드에 pip 없이 python3만 있으면 되는 쪽 선택)
  - `GET /api/events` — `SavedClips`/`SentryClips` 이벤트 목록 + `event.json`
    메타(시각/장소/사유) + 카메라별 클립·크기 (최신순)
  - `GET /files/<타입>/<이벤트>/<파일>` — mp4/thumb.png, **byte-range 지원**
  - `GET /healthz` — 배포 셀프체크 (아카이브 루트 접근 여부 포함)
  - `RecentClips`/`EncryptedClips`/`Photobooth`는 화이트리스트로 차단(설계대로)
- **검증 상태**: `python3 pi/test_clipserver.py` 20개 테스트 통과(healthz·빈아카이브
  감지·목록·정렬·메타 폴백·Range 206/416·이어받기 재조립·경로 탈출 차단).
  **보드 배포·실차 데이터 검증은 미완**
- 배포물: `pi/clipserver.service` + `pi/install_clipserver.sh` — 보드 checkout에서
  `sudo pi/install_clipserver.sh` 한 번으로 `/mutable` 설치(읽기전용 루트 대응),
  systemd enable/start, `/healthz` 확인까지 수행
- **셀프체크:** 폰 브라우저로 이벤트 목록 JSON과 파일 다운로드가 되는지 ← 보드 배포 후

### Phase 2 — Android 앱: 클립 브라우저 ✅ 재작성됨 (2026-07-12, 실기 검증은 아직)
- CompanionDeviceManager로 보드(BLE) association + presence 관찰
  → 근접 시 WiFi 미리 연결(자동 다운로드는 안 함, 그냥 "준비"만) + "클립 보기" 알림
- **이벤트 목록 화면**(`ClipBrowserActivity`): SavedClips/SentryClips를
  시각·장소·사유(event.json)로 표시 (thumb.png 썸네일은 Phase 3로 미룸)
- **선택 다운로드**: 이벤트 탭 → 카메라 고르기(front 우선 정렬, 세그먼트 수·용량 표시)
  → 그 카메라 파일만 받아 MediaStore.Video 저장(`DownloadService`, IS_PENDING으로
  부분 파일이 갤러리에 안 보이게)
- **다운로드 신뢰성**(2026-07-12): HTTP status 검증(200/206 아니면 실패),
  받은 바이트 == 총 크기 검증(연결 끊김에 의한 조용한 잘림 차단), 클립당 3회
  재시도(백오프) — 잘린 클립이 '완료'로 저장되는 것을 막음
- 옛 `SyncService`(자동 풀다운로드)·`NeoApi`(존재하지 않는 neo API 가정)는 삭제됨
  → `ClipApi`(clipserver 계약) + `PiNet`(핫스팟 연결 공용화)로 대체
- **검증 상태**: `assembleDebug` 빌드 통과(재작성 시점). 이후 다운로드 신뢰성
  변경은 **코드 인스펙션으로만 검증**(비대화형 세션이라 재빌드 미실행). **실기(폰+보드)
  연결 검증은 미완**
...
- 이벤트 목록에 thumb.png 썸네일 표시 (API는 이미 서빙함 — 앱 UI만 남음)
